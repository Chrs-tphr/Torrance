/*===================================================================
// Batch Script Name: LICENSE_EXPIRATIONS.js
===================================================================*/

/*
// Testing values.  Replace with batch parameters when testing is complete
aa.env.setValue("fromDate","04/22/2011")
aa.env.setValue("toDate","06/09/2012")
...
aa.env.setValue("setPrefix","L_ABT");
...
aa.env.setValue("feeList","LIC_CONT_REN");
aa.env.setValue("feeSched","LIC_CONTRACTOR_RENEWAL");
aa.env.setValue("feePeriod","FINAL");
*/
...
| Customized for Torrance - DMH
| added debug info to existing for addRenewalFeesBatch 20151112
/------------------------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------------------------/
|
...
/------------------------------------------------------------------------------------------------------*/
emailText = "";
maxSeconds = 660 * 60;		// number of seconds allowed for batch processing, usually < 5*60
message = "";
br = "<br>";
/*------------------------------------------------------------------------------------------------------/
| BEGIN Includes
...
/* 
// Testing values.  Replace with batch parameters when testing is complete
aa.env.setValue("fromDate","09/01/2013")
aa.env.setValue("toDate","01/01/2014")
aa.env.setValue("appGroup","Licenses");
aa.env.setValue("appTypeType","*");
aa.env.setValue("appSubtype","*");
		...
		if (createRenewalRecord && createRenewalRecord.substring(0,1).toUpperCase().equals("Y")) {
			...
			if (!createResult.getSuccess) 
				...
				if (renewalCap.isCompleteCap())	{
					...
					// add fees 
					addRenewalFeesBatch(appTypeArray, renewalCapId, capId);	
					//if (feeList.length > 0) {
...
function addRenewalFeesBatch(appTypeArray, renewalCapId, parentLicId) {
// called from batch - does the initial add
	if (appTypeArray[1] == "Business") {
		addFee("STATE", "LIC_GENERAL", "STANDARD", 1, "N", renewalCapId);
		actCode = getAppSpecific("Activity Code", parentLicId);
		logDebug("license expirations script addRenewalFeesBatch Activity Code = " + actCode);
		...
		if (feeDriverString && feeDriverString != "") {
			...
			for (pIndex in feeDriverPieces) {
				...
				pieces = driver.split("=");
				feeItem = pieces[0]; feeCode = pieces[1];
				switch ("" + feeItem) {
					case "EXEMPT":
					...
					case "BASE":
						if (feeCode && feeCode != "" && feeCode != "NONE") {
							addFee(feeCode, "LIC_GENERAL", "STANDARD", 1, "N", renewalCapId);
						}
						break;
					case "PERPERSON":
						if (feeCode && feeCode != "" && feeCode != "NONE") {
							var numPersons = getAppSpecific("Number of Persons Working in Torrance", parentLicId);
							if (numPersons && numPersons != "") {
								...
								if (busInTor && busInTor != "" && busInTor.substring(0,1).toUpperCase() == "Y")
									addFee("LIC_001", "LIC_GENERAL", "STANDARD", parseInt(numPersons), "N", renewalCapId);
							}
							else {
								addFee(feeCode, "LIC_GENERAL", "STANDARD", parseInt(numPersons), "N", renewalCapId);
							}}
						}
						break;
					case "PERUNIT":
						if (feeCode && feeCode != "" && feeCode != "NONE") {
							var numUnits = getAppSpecific("Units", parentLicId);
                                                         if (numUnits && numUnits != "") {
							addFee(feeCode, "LIC_GENERAL", "STANDARD", parseInt(numUnits), "N", renewalCapId);
                                                        }
						}
						break;
					case "VENDING":
						assessVendingMachineFees(false, "add", renewalCapId, parentLicId);
						break;
					case "COMMERCIAL":
						SqFt = getAppSpecific("Square Footage", parentLicId);
						if (SqFt && SqFt != "") {
							addFee("LIC_9999", "LIC_GENERAL", "STANDARD", parseFloat(SqFt), "N", renewalCapId);
							}
						break;
					default :
		...
		if (hasIssuedDanceEndorsement(parentLicId)) {
			addFee("DANCE_001", "DANCE", "STANDARD", 1, "N", renewalCapId);
		}
		if (hasIssuedEntEndorsement(parentLicId)) {
			addFee("ENT_001", "ENTERTAINMENT", "STANDARD", 1, "N", renewalCapId);
		}
			
	}
	
	if (appTypeArray[1] == "Renewable") {
		if (appTypeArray[2] == "Alarm System") {
			resCom = getAppSpecific("Commercial/Residential", parentLicId);
			if (resCom && resCom != "" && resCom != "Senior Citizen")
				addFee("ALARM_002", "ALARM", "STANDARD", 1, "N", renewalCapId);
		}
		if (appTypeArray[2] == "Solicitor") {
			addFee("SOL_001", "SOLICITOR", "STANDARD", 1, "N", renewalCapId);
		}
		if (appTypeArray[2] == "Swap Meet") {
			addFee("SWP_001", "SWAP_MEET", "STANDARD", 1, "N", renewalCapId);
			numSpaces = getAppSpecific("Number of Spaces", parentLicId);
			if (numSpaces && numSpaces != "")
				addFee("SWP_002", "SWAP_MEET", "STANDARD", parseInt(numSpaces), "N", renewalCapId);
		}
		if (appTypeArray[2] == "Taxicab Driver") {
			addFee("TAXI_001", "TAXI", "STANDARD", 1, "N", renewalCapId);
		}
		if (appTypeArray[2] == "Taxicab Franchise") {
			addFee("STATE", "LIC_GENERAL", "STANDARD", 1, "N", renewalCapId);
			addFee("TXICO_002", "TAXII_FRANCHISE", "STANDARD", 1, "N", renewalCapId);
			numTaxis = getAppSpecific("Number of Taxis", parentLicId);
			if (numTaxis && numTaxis != "")
				addFee("TXICO_003", "TAXII_FRANCHISE", "STANDARD", parseInt(numTaxis), "N", renewalCapId);
		}
	}
}
